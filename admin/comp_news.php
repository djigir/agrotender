...
function checkDtFormat($dt)
{
	...
	$PAGE_HEADER['ru'] = "Редактировать Новости Компаний";
	...
	$THIS_TABLE = $TABLE_COMPANY_NEWS; //$TABLE_COMMENT;
...
<?php
	switch( $action )
	{
		case "delete":
			...
			{
    			if(!mysqli_query($upd_link_db,"DELETE FROM $TABLE_COMPANY_NEWS WHERE id=".$com_id[$i]." "))
				{
		...
		case "deleteitem":
            ...
            if(!mysqli_query($upd_link_db,"DELETE FROM $TABLE_COMPANY_NEWS WHERE id=".$item_id." "))
			{
        		echo "<b>".mysqli_error($upd_link_db)."</b>";
    		}
    		break;
		...
		case "update":
			...
			if(!mysqli_query($upd_link_db,"UPDATE $TABLE_COMPANY_NEWS SET title='$newstitle', content='".addslashes($newscont)."' WHERE id='".$item_id."'"))
			{
				echo "<b>".mysqli_error($upd_link_db)."</b>";
			}
			break;
    ...
    if( $mode == "edit" )
    {
		...
		if($res = mysqli_query($upd_link_db,"SELECT m1.*, DAYOFMONTH(m1.add_date) as dd, MONTH(m1.add_date) as dm, YEAR(m1.add_date) as dy FROM $TABLE_COMPANY_NEWS m1 WHERE m1.id='$item_id'"))
		{
...
<?php
    	$found_news = 0;
		if( $res = mysqli_query($upd_link_db,"SELECT m1.* FROM $TABLE_COMPANY_NEWS m1 ORDER BY m1.add_date DESC") )
		{
