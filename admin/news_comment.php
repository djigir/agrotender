...
function checkDtFormat($dt)
{
	...
	$THIS_TABLE = $TABLE_NEWS_COMMENT; //$TABLE_COMMENT;
	$THIS_TABLE_LANG = $TABLE_NEWS_COMMENT_LANGS; //$TABLE_COMMENT_LANGS;
?>
...
<?php
			...
			ORDER BY m1.add_date DESC") )
		{
			...
			{
				$prod_name = "";
				$query = "SELECT e1.*, e2.title, e2.content FROM $TABLE_NEWS e1
					INNER JOIN $TABLE_NEWS_LANGS e2 ON e1.id=e2.news_id AND e2.lang_id='$LangId'
