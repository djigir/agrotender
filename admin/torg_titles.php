<?php
	...
	$cult_id = GetParameter("cult_id", 0);
	$trade = GetParameter("trade", 1);
	...
	switch( $action )
	{
		case "update":
			...
			if( $org_id != 0 )
			{
				...
				if(!mysqli_query($upd_link_db,"UPDATE $TABLE_TORG_TITLES SET
			...
			else
			{
				...
				if( $cult_id == 0 )
				{
					if( !mysqli_query($upd_link_db,"INSERT INTO $TABLE_TORG_TITLES (page_title, page_keywords, page_descr, content_text, content_words,
						modify_date, obl_id, trade, cult_id, add_date, lang_id,
						...
						VALUES ('".addslashes($orgtitle)."','".addslashes($orgkeyw)."','".addslashes($orgdescr)."',
							'".addslashes($orgcont)."', '".addslashes($orgwords)."', NOW(), '".$item_id."', '".$trade."', 0, NOW(), '$LangId',
				...
				else
				{
					if( !mysqli_query($upd_link_db,"INSERT INTO $TABLE_TORG_TITLES (page_title, page_keywords, page_descr, content_text, content_words,
						...
						sortmode_id)
						VALUES ('".addslashes($orgtitle)."','".addslashes($orgkeyw)."','".addslashes($orgdescr)."',
							'".addslashes($orgcont)."', '".addslashes($orgwords)."', NOW(), '".$item_id."', '".$trade."', '".$cult_id."', NOW(), '$LangId',
							'' ,'', '', '', '',
							'".addslashes($sort_id)."') ") )
					{
						echo mysqli_error($upd_link_db);
	...
	if( $mode == "edit" )
	{
		...
		$query = "SELECT DISTINCT sortmode_id FROM $TABLE_TORG_TITLES
			WHERE obl_id='$item_id' AND trade='$trade' AND cult_id='$cult_id' AND lang_id='$LangId'";
		...
		/*
		$filters = Array();
		$query = "SELECT DISTINCT filter_id FROM $TABLE_TORG_TITLES
		...
		*/
		...
		if( $cult_id == 0 )
		{
			$query = "SELECT ct1.* FROM $TABLE_TORG_TITLES ct1
			WHERE ct1.obl_id='$item_id' AND ct1.trade='$trade' AND ct1.cult_id=0 AND ct1.sortmode_id='$sort_id' AND ct1.lang_id='$LangId'";
		}
		else
		{
			$query = "SELECT ct1.*, c1.type_name as cult FROM $TABLE_TORG_TITLES ct1
			INNER JOIN $TABLE_TORG_PROFILE_LANGS c1 ON ct1.cult_id=c1.profile_id AND c1.lang_id=$LangId
			WHERE ct1.obl_id='$item_id' AND ct1.trade='$trade' AND ct1.cult_id='$cult_id' AND ct1.sortmode_id='$sort_id' AND ct1.lang_id='$LangId'";
		}
		...
		if($res = mysqli_query($upd_link_db,$query))
		{
			if($row = mysqli_fetch_object($res))
			{
				...
				$cultname = ($cult_id != 0 ? stripslashes($row->cult) : "");
		...
		if( $orgtitle == "" )
		{
			...
			*/
			...
			if( $cult_id != 0 )
			{
				$query = "SELECT * FROM $TABLE_TORG_PROFILE_LANGS WHERE profile_id='".$cult_id."' AND lang_id='".$LangId."'";
				if( $res = mysqli_query($upd_link_db, $query ) )
				{
					if( $row = mysqli_fetch_object( $res ) )
					{
						$cultname = stripslashes($row->type_name);
					}
					mysqli_free_result( $res );
				}
			}
		...
		$gen_title = Torg_SeoDefTitle($LangId, $REGIONS[$item_id], $trade, $cult_id, $cultname, $sort_id);
	...
	if( count($sortby_arr2) > 1 )
	{
		...
		for($i=0; $i<count($sortby_arr2); $i++ )
		{
			...
			if( $sort_id == $sortby_arr2[$i]['id'] )
				...
				echo '<a href="'.$PHP_SELF.'?mode=edit&item_id='.$item_id.'&trade='.$trade.'&cult_id='.$cult_id.'&sort_id='.$sortby_arr2[$i]['id'].'">'.$sortby_arr2[$i]['name'].'</a>';
		}
		echo '</div>';
	}
	*/

	$trade_arr = Array( Array("id" => 1, "name" => "Закупки"), Array("id" => 2, "name" => "Продажи") );
	...
	for($i=0; $i<count($trade_arr); $i++ )
	{
		...
		if( $trade == $trade_arr[$i]['id'] )
			echo '<b>'.$trade_arr[$i]['name'].'</b>';
		else
			echo '<a href="'.$PHP_SELF.'?mode=edit&item_id='.$item_id.'&trade='.$trade_arr[$i]['id'].'&cult_id='.$cult_id.'">'.$trade_arr[$i]['name'].'</a>';
	}
	echo '</div>';
		...
		for($i=0; $i<count($filters); $i++ )
		{
			...
			for( $j=0; $j<count($opts); $j++ )
			{
				...
				if( ($filt_id == $filters[$i]['id']) && ($filt_val == $opts[$j]['id']) )
					...
					echo '<a href="'.$PHP_SELF.'?mode=edit&item_id='.$item_id.'&trade='.$trade.'&cult_id='.$cult_id.'&sort_id=0">'.$opts[$j]['name'].'</a>';
			}
...
?>
	...
	<input type="hidden" name="cult_id" value="<?=$cult_id;?>" />
...
<?php
	...
	{
		echo "<tr>
			<td class=\"fr\" style=\"padding: 1px 10px 1px 20px\">".$REGIONS[$i]."</td>
			<td><a href=\"$PHP_SELF?item_id=".$i."&mode=edit\"><img src=\"img/edit.gif\" width=\"20\" height=\"20\" border=\"0\" alt=\"Редактировать\" /></a>&nbsp;</td>
		</tr>";

		for( $j=0; $j<count($cult_list); $j++ )
		{
			echo "<tr>
				<td class=\"fr\" style=\"padding: 1px 10px 1px 40px\">".$cult_list[$j]['name']."</td>
				<td><a href=\"$PHP_SELF?item_id=".$i."&cult_id=".$cult_list[$j]['id']."&mode=edit\"><img src=\"img/edit.gif\" width=\"20\" height=\"20\" border=\"0\" alt=\"Редактировать\" /></a>&nbsp;</td>
			</tr>";
		}
	}
?>
