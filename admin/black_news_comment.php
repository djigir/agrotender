...
function checkDtFormat($dt)
{
	...
	$newsid = GetParameter("newsid", 0);

	$datest = GetParameter("datest", date("d.m.Y", time()));

	$THIS_TABLE = $TABLE_BLNEWS_COMMENT; //$TABLE_COMMENT;
	$THIS_TABLE_LANG = $TABLE_BLNEWS_COMMENT_LANGS; //$TABLE_COMMENT_LANGS;
?>
...
<?php
			...
			ORDER BY m1.add_date DESC") )
		{
			...
			{
				$prod_name = "";
				$query = "SELECT e1.*, e2.title, e2.content FROM $TABLE_BLNEWS e1
					INNER JOIN $TABLE_BLNEWS_LANGS e2 ON e1.id=e2.news_id AND e2.lang_id='$LangId'
					WHERE e1.id='".$row->item_id."'";
				...
				echo "<tr>
					...
					<td align=\"center\">
						<a href=\"$PHP_SELF?action=deleteitem&ntype=".$ntype."&newsid=".$newsid."&item_id=".$row->id."\"><img src=\"img/delete.gif\" width=\"20\" height=\"20\" border=\"0\" alt=\"".$strings['tipdel'][$lang]."\" /></a>&nbsp;
						<a href=\"$PHP_SELF?mode=edit&ntype=".$ntype."&newsid=".$newsid."&item_id=".$row->id."\"><img src=\"img/edit.gif\" width=\"20\" height=\"20\" border=\"0\" alt=\"".$strings['tipedit'][$lang]."\" /></a>&nbsp;
					</td>
				</tr>
