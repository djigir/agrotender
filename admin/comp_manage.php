...
function checkDtFormat($dt)
{
	...
	switch( $action )
	{
		...
		case "update":
			...
			$price_forward_avail = intval(GetParameter("price_forward_avail", 0));
			$premium_forward = intval(GetParameter("premium_forward", 0));
			$sort_forward = intval(GetParameter("sort_forward", 0));
			...
			$query = "UPDATE $TABLE_COMPANY_ITEMS SET 
				...
				trader_price_forward_avail=$price_forward_avail, trader_premium_forward=$premium_forward, trader_sort_forward=$sort_forward,
    ...
    if( $mode == "edit" )
    {
		...
		if($res = mysqli_query($upd_link_db,"SELECT m1.* FROM $TABLE_COMPANY_ITEMS m1 WHERE m1.id='$item_id'"))
		{
			if($row = mysqli_fetch_object($res))
			{
				...
				$price_forward_avail = $row->trader_price_forward_avail;
				$sort_forward = $row->trader_sort_forward;
				$premium_forward = $row->trader_premium_forward;
...
?>
	...
	Премиум:<select name="newstraderprem" style="margin-right: 10px">
		...
		<option value="2"<?=($newstraderprem == 2 ? " selected" : "");?>>Премиум +</option>
	</select>
...
?>
	...
	Премиум:<select name="newstraderprem2" style="margin-right: 10px">
		...
		<option value="2"<?=($newstraderprem2 == 2 ? " selected" : "");?>>Премиум +</option>
	</select>
	Приоритет: <input type="text" name="newstradersort2" size="2" value="<?=$newstradersort2;?>" />
	</td></tr>

	<tr><td class="ff">Таблица форвардов:</td><td class="fr"> Активна: <select name="price_forward_avail" style="margin-right: 10px">
		<option value="0"<?=($price_forward_avail == 0 ? " selected" : "");?>>Нет</option>
		<option value="1"<?=($price_forward_avail == 1 ? " selected" : "");?>>Да</option>
	</select>
	Премиум:<select name="premium_forward" style="margin-right: 10px">
		<option value="0"<?=($premium_forward == 0 ? " selected" : "");?>>Нет</option>
		<option value="1"<?=($premium_forward == 1 ? " selected" : "");?>>Да</option>
	</select>
	Приоритет: <input type="text" name="sort_forward" size="2" value="<?=$sort_forward;?>" />
	</td></tr>
...
<?php
	...
	$count = 0;
        ...
        {
        	echo "<tr><td align=\"center\" colspan=\"17\"><div style=\"float:left;\">Всего:".$count."</div><input type=\"submit\" name=\"delete_but\" value=\" ".$strings['btndel'][$lang]." \" /> <input type=\"submit\" name=\"refresh_but\" value=\" ".$strings['btnrefresh'][$lang]." \" /></td></tr>";
        }
    ?>
    </form>
    </table>
