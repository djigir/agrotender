...
function checkDtFormat($dt)
{
	...
	$strings['tipedit']['ru'] = "Редактировать эту Технологию";
   	$strings['tipdel']['ru'] = "Удалить эту Технологию";
   	$strings['hdrlist']['ru'] = "Список Технологий";
   	$strings['hdradd']['ru'] = "Добавить Технологию";
   	$strings['hdredit']['ru'] = "Редакировать Технологию";
	...
	$strings['btnadd']['ru'] = "Добавить";
   	...
   	$strings['nolist']['ru'] = "В базе нет технологий";
	$strings['rowcont']['ru'] = "Содержание записей";
   	$strings['rowfunc']['ru'] = "Функции";

	$PAGE_HEADER['ru'] = "Редактировать Технологии";
	...
	$THIS_TABLE = $TABLE_TECHNOLOGY;
	$THIS_TABLE_LANG = $TABLE_TECHNOLOGY_LANGS;
?>
	...
	switch( $action )
	{
    	case "add":
    		...
    		$query = "INSERT INTO $THIS_TABLE ( dtime, first_page, filename_src)
    			VALUES ( NOW(), '1', '".addslashes($myfile)."')";
		...
		case "update":
			...
			if(!mysqli_query($upd_link_db,"UPDATE $THIS_TABLE SET first_page='1', filename_src='".addslashes($myfile)."', dtime='$db_datest'
				WHERE id='".$item_id."'"))
			{
				echo "<b>".mysqli_error($upd_link_db)."</b>";
			}
...
?>
    ...
    <input type="hidden" name="ntype" value="<?=$ntype;?>" />
    <tr><th>&nbsp;</th><th style="padding: 1px 10px 1px 20px" width="65%"><?=$strings['rowcont'][$lang];?></th><th><?=$strings['rowfunc'][$lang];?></th></tr>
<?php
    	$found_news = 0;
		if( $res = mysqli_query($upd_link_db,"SELECT m1.*, m2.title, m2.content
			FROM $THIS_TABLE m1, $THIS_TABLE_LANG m2
