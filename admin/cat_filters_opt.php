<?php
	...
	$strings['tipedit']['en'] = "Edit vehicle params";
	 ...
	 $strings['tipedit']['ru'] = "Редактировать фильтр для товаров";
   	$strings['tipdel']['ru'] = "Удалить значение фильтра из списка";
	...
	$THIS_TABLE_OPTS = $TABLE_CAT_PROFILE_FILT_OPTIONS;
	$THIS_TABLE_OPTS_LANG = $TABLE_CAT_PROFILE_FILT_OPTIONS_LANGS;
	...
	$disptypes[] = Array();
	$disptypes_num = 0;
	$param_id = GetParameter("param_id", "");
	...
	$orgname = GetParameter("orgname", 0);
	...
	case "change":
            ...
            if( $apply_but != "" )
            {
            	...
            	for( $i=0; $i<count($option_id); $i++ )
            	{
	                ...
	                if( !mysqli_query($upd_link_db, "UPDATE $THIS_TABLE_OPTS_LANG
	                    SET option_text='".addslashes($orgname[$i])."', link_text='".addslashes($orgname2[$i])."'
	    ...
	    if( !mysqli_query($upd_link_db, "UPDATE $THIS_TABLE_P2P SET sort_ind='".$ordsort_ind[$i]."' WHERE id='".$p2p_id[$i]."'" ) )
                	{
                   		echo mysqli_error($upd_link_db);
                	}
			break;
    ...
    if( $mode == "edit" )
    {
		...
		if($res = mysqli_query($upd_link_db,"SELECT p1.*, p2.name, p2.izm
			FROM $THIS_TABLE p1, $THIS_TABLE_LANG p2
...
?>
    ...
    <h3>Список опций фильтра &quot;<?=$param_name;?>&quot;</h3>
...
<?php
    	$found_items = 0;
		if( $res = mysqli_query($upd_link_db,"SELECT o1.*, o2.option_text, o2.link_text
			...
			while($row=mysqli_fetch_object($res))
			{
            	...
            	echo "<tr>
            			...
            			<td><input type=\"hidden\" name=\"option_id[]\" value=\"".$row->id."\" />
                     	...
                     	<td><input type=\"text\" name=\"orgname2[]\" size=\"20\" value=\"".stripslashes($row->link_text)."\"></td>";
                if( $param_is_filter )
                {
                	$ico = "";
                	...
                	echo '<td>'.($ico != "" ? '<img src="'.$WWWHOST.$FILE_DIR.$ico.'" alt="" />' : ' &nbsp; ').'</td>';
...
<tr>
	...
	<td><input type=\"button\" onclick=\"javascript:Delet('$PHP_SELF?action=deleteitem&item_id=".$row['param_id']."');\" value=\"Удалить\"></td>
		...
		if( $found_items == 0 )
		{
			echo "<tr><td colspan=\"".( $param_is_filter ? 7 : 5 )."\" align=\"center\"><br />В базе нет ни одной опции для фильтра<br /><br /></td></tr>
    ...
    <br /><br />
    <h3>Добавить Новую Опцию для Фильтра &quot;<?=$param_name;?>&quot;</h3>
    <table align="center" cellspacing="0" cellpadding="1" border="0" class="tableborder">
    <tr><td>
    	<table width="100%" cellspacing="1" cellpadding="1" border="0">
    ...
    <input type="hidden" name="profid" value="<?=$profid;?>" />
    <input type="hidden" name="param_id" value="<?=$param_id;?>">
    <tr><td class="ff">Название опции:</td><td class="fr"><input type="text" size="70" name="orgname" value="" /></td></tr>
    <tr><td class="ff">Порядковый номер:</td><td class="fr"><input type="text" size="3" name="orgsort" value="" /></td></tr>
    <tr><td class="fr" colspan="2" align="center"><input type="submit" value=" Добавить "></td></tr>
    </form>
    	</table>
    	</td></tr>
    </table>
<p align="center"><a href="<?$WWWHOST;?>cat_filters.php?profid=<?=$profid;?>">Вернуться к фильтрам</a></p>
<?php
    }
