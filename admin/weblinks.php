<?php
    ...
    $strings['tipedit']['ru'] = "Редактировать ссылку";
   	$strings['tipdel']['ru'] = "Удалить ссылку из списка";

	$PAGE_HEADER['ru'] = "Редактировать Список Полезных Ссылок";
...
<?php
	...
	switch( $action )
	{
    	case "add":
    		...
    		$query = "INSERT INTO $TABLE_WEBLINKS ( weburl, email, phone, sort_num, logo_filename)
			...
			else
			{
                ...
                {
                	if( !mysqli_query($upd_link_db, "INSERT INTO $TABLE_WEBLINKS_LANGS ( item_id, lang_id, orgname, descr )
		...
		case "delete":
			...
			{
    			if(!mysqli_query($upd_link_db,"DELETE FROM $TABLE_WEBLINKS WHERE id=".$items_id[$i]." "))
				{
        			echo "<b>".mysqli_error($upd_link_db)."</b>";
    			}
    			else
    			{
    				if(!mysqli_query($upd_link_db,"DELETE FROM $TABLE_WEBLINKS_LANGS WHERE item_id=".$items_id[$i]." "))
					{
	        			echo "<b>".mysqli_error($upd_link_db)."</b>";
	    			}
    			}
		...
		case "deleteitem":
            ...
            if(!mysqli_query($upd_link_db,"DELETE FROM $TABLE_WEBLINKS WHERE id=".$item_id." "))
			{
        		echo "<b>".mysqli_error($upd_link_db)."</b>";
    		}
    		else
    		{
    			if(!mysqli_query($upd_link_db,"DELETE FROM $TABLE_WEBLINKS_LANGS WHERE item_id=".$item_id." "))
				{
        			echo "<b>".mysqli_error($upd_link_db)."</b>";
    			}
    		}
		...
		case "update":
			...
			if(!mysqli_query($upd_link_db,"UPDATE $TABLE_WEBLINKS SET phone='".addslashes($orgphone)."', email='".addslashes($orgemail)."',
			...
			if(!mysqli_query($upd_link_db,"UPDATE $TABLE_WEBLINKS_LANGS SET orgname='".addslashes($orgname)."',
    ...
    if( $mode == "edit" )
    {
		...
		if($res = mysqli_query($upd_link_db,"SELECT l1.*, l2.orgname, l2.descr FROM $TABLE_WEBLINKS l1
			INNER JOIN $TABLE_WEBLINKS_LANGS l2 ON l1.id=l2.item_id AND l2.lang_id='$LangId'
			WHERE l1.id=$item_id"))
		{
