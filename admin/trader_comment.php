...
function checkDtFormat($dt)
{
	...
	$PAGE_HEADER['ru'] = "Редактировать Отзывы Трейдеров";
	...
	$THIS_TABLE = $TABLE_TRADER_COMMENT; //$TABLE_COMMENT;
	$THIS_TABLE_LANG = $TABLE_TRADER_COMMENT_LANGS; //$TABLE_COMMENT_LANGS;
?>
...
<?php
			...
			ORDER BY m1.add_date DESC") )
		{
			...
			{
				$prod_name = "";
				$query = "SELECT e1.*, e2.name FROM $TABLE_TRADER e1
					INNER JOIN $TABLE_TRADER_LANGS e2 ON e1.id=e2.item_id AND e2.lang_id='$LangId'
					WHERE e1.id='".$row->item_id."'";
				if( $result= mysqli_query($upd_link_db,$query) )
				{
					if( $myrow = mysqli_fetch_object($result) )
					{
						$prod_name = stripslashes($myrow->name);
					}
