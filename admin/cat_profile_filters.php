<?php
	...
	$query = "SELECT f1.id, f1.param_display_type_id, f2.name
		FROM $TABLE_CAT_PROFILE_FILTERS p1
		INNER JOIN $TABLE_CAT_FILTERS f1 ON p1.param_id=f1.id
		INNER JOIN $TABLE_CAT_FILTERS_LANGS f2 ON f1.id=f2.param_id AND f2.lang_id='$LangId'
                ...
                for( $j=0; $j<count($filters); $j++ )
				{
					...
					switch( $filters[$j]['disptype'] )
					{
						case $FIELD_TYPE_SELECT:
							...
							$query = "SELECT * FROM $TABLE_CAT_FILTER_VALUES WHERE item_id='$prod_id' AND param_id='".$filters[$j]['id']."'";
						    if( $res = mysqli_query($upd_link_db, $query ) )
						    {
							    if( $row = mysqli_fetch_object($res) )
							    {
							    	// The filter for product exists
							    	$query = "UPDATE $TABLE_CAT_FILTER_VALUES SET value='".$curfval."' WHERE id='".$row->id."'";
							    	//echo $query."<br />";
						...
						case $FIELD_TYPE_FLAG:
						    ...
						    if( $res = mysqli_query($upd_link_db, $query ) )
						    {
							    ...
							    {
							    	// The filter for product exists
							    	if( !mysqli_query($upd_link_db, "UPDATE $TABLE_CAT_FILTER_VALUES SET value='".$curfval."' WHERE id='".$row->id."'" ) )
									{
									   echo mysqli_error($upd_link_db);
									}
								}
	...
	echo '<h3>Редактирование фильтров товаров типа &quot;'.$profile_name.'&quot;</h3>';
	...
	if( $res = mysqli_query($upd_link_db,$query) )
	{
		while( $row = mysqli_fetch_object($res) )
		{
			...
			for( $i=0; $i<count($filters); $i++ )
			{
				...
				$query_v = "SELECT id, value FROM $TABLE_CAT_FILTER_VALUES WHERE param_id='".$filters[$i]['id']."' AND item_id='".$row->id."'";
				...
				{
					echo '<td>';
					$query_o = "SELECT o2.option_text, o.id FROM $TABLE_CAT_PROFILE_FILT_OPTIONS o
							INNER JOIN $TABLE_CAT_PROFILE_FILT_OPTIONS_LANGS o2 ON o2.option_id=o.id AND o2.lang_id='$LangId'
