...
function checkDtFormat($dt)
{
	...
	$THIS_TABLE = $TABLE_NEWS;
	$THIS_TABLE_LANG = $TABLE_NEWS_LANGS;
?>
...
<?php
	...
	switch( $action )
	{
    	case "add":
    		...
    		$query = "INSERT INTO $THIS_TABLE ( dtime, ngroup, first_page, intop, filename_src)
    			VALUES ( NOW(), '$ntype', '$newsfirst', '$newstop', '".addslashes($myfile)."')";
			if( mysqli_query($upd_link_db,$query) )
			{
	            ...
	            // Make relinkin
	            $relnews = Array();
	            $query = "SELECT * FROM $TABLE_NEWS ORDER BY dtime DESC LIMIT 0,5";
		...
		case "update":
			...
			if(!mysqli_query($upd_link_db,"UPDATE $THIS_TABLE SET first_page='$newsfirst', intop='$newstop', filename_src='".addslashes($myfile)."', dtime='$db_datest'
				WHERE id='".$item_id."'"))
			{
				echo "<b>".mysqli_error($upd_link_db)."</b>";
			}
...
<?php
			...
			LIMIT ".(($pi-1)*$pn).",$pn") )
		{
			while($row=mysqli_fetch_object($res))
			{
				...
				$comment_num = 0;
	            $query1 = "SELECT count(*) as commentnum FROM $TABLE_NEWS_COMMENT WHERE item_id='".$row->id."'";
				...
				//echo "<br><br>".$str_text;
							echo "</td>
                               ...
                               <td align=\"center\">".($comment_num == 0 ? "0" : '<a href="news_comment.php?newsid='.$row->id.'">'.$comment_num.'</a>')."</td>
                               <td align=\"center\">
