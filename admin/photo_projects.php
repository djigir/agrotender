<?php
	...
	$strings['tipedit']['en'] = "Edit информацию о проекте";
   	$strings['tipdel']['en'] = "Delete this galery";
   	$strings['tipphoto']['en'] = "Edit photos list";

    $strings['tipedit']['ru'] = "Редактировать список проектов";
   	$strings['tipdel']['ru'] = "Удалить группу проектов со всеми фотографиями";
   	$strings['tipphoto']['ru'] = "Редактировать фотографии проекта";

	$PAGE_HEADER['ru'] = "Редактировать описания проекты";
		...
		case "addphoto":
            ...
            }
            {
				...
				else
				{
	   			...
	   			// Make ico photo
            	$res = ResizeImage( $photofile['tmp_name'], "../".$iconame, strtolower($newfileext), ".jpg", $GALERY_I_W, $GALERY_I_H, false, $JPEG_COMPRESS_RATIO);
				if( !$res )
				{
...
?>
		...
		<h3>Редактировать информацию о проекте</h3>
		...
		<table align="center" width="550" cellspacing="0" cellpadding="1" border="0" class="tableborder">
			<tr><td>
		        ...
		        <tr>
					<td colspan="2" class="fh">Информация о проекте</td>
				</tr>
		        <tr>
					<td class="ff">Название проекта:</td>
					<td class="fr">
		                <input type="text" name="phototitle" size="40" value="<?=$photo['alt'];?>" />
					</td>
				</tr>
				<tr>
					<td class="ff">Описание проекта:</td>
					<td class="fr">
...
<?php
		if( count($photos) > 0 )
		{
			...
			for($i=0; $i<count($photos); $i++)
			{
				...
				echo "<td align=\"center\"><div style=\"border: 1px solid #b0b0b0; height: 200px;\">";
		...
		{
			echo "<tr><td align=\"center\">Нет фотографий в данной проекте</td></tr>";
		}
?>
		</table>
		<br />
		<center><a href="<?=$PHP_SELF;?>">Вернуться к списку групп проектов</a></center>

		<h3>Добавить фото к проекту</h3>
		<form action="<?=$PHP_SELF;?>" method="POST" name="addfrm" id="addfrm" enctype="multipart/form-data">
		<input type="hidden" name="item_id" value="<?=$item_id;?>" />
		...
		<table align="center" width="550" cellspacing="0" cellpadding="1" border="0" class="tableborder">
			<tr><td>
		        <table width="100%" cellspacing="1" cellpadding="1" border="0">
		        <tr>
					<td colspan="2" class="fh">Добавить фото к проекту</td>
				</tr>
		        <tr>
					<td class="ff">Название:</td>
					<td class="fr">
...
?>
	...
	<input type="hidden" name="item_id" value="<?=$item_id;?>" />
    <tr><td class="ff">Название проекта:</td><td class="fr"><input type="text" size="70" name="orgname" value="<?=$orgname;?>" /></td></tr>
    <tr>
		<td class="ff">Описание:</td>
...
<script language="javascript1.2">
	...
	<tr><td class="ff">Тип проета:</td>
		<td class="fr">
			<select name="orgfirst">
			<option value="0" <?=($orgfirst == 0 ? "selected" : "");?>>Завершенные проекты</option>
			<option value="1" <?=($orgfirst == 1 ? "selected" : "");?>>Концепции проектов</option>
			</select>
		</td></tr>
...
?>
    ...
    <tr><th>&nbsp;</th><th>Картинка</th><th>Название проекта</th><th>Фото</th><th>Функции</th></tr>
    <?
    	$found_items = 0;
    ...
    <br /><br />
    <h3>Добавить Новую Группу Проектов</h3>
    <table align="center" cellspacing="0" cellpadding="1" border="0" class="tableborder">
    <tr><td>
    	<table width="100%" cellspacing="1" cellpadding="1" border="0">
    <form action="<?=$PHP_SELF;?>" name="catfrm" method="POST">
    <input type="hidden" name="action" value="add">
    <tr><td class="ff">Название проета:</td><td class="fr"><input type="text" size="70" name="orgname"></td></tr>
    <tr>
		<td class="ff">Описание:</td>
...
<script language="javascript1.2">
	...
	<tr><td class="ff">Тип проекта:</td>
		<td class="fr">
			<select name="orgfirst">
			<option value="0">Завершенные проекты</option>
			<option value="1">Концепции проектов</option>
			</select>
		</td></tr>
