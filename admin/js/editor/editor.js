...
this.replaceNextlines = 1; // replace nextlines from spaces (on output)
this.plaintextInput = 1;   // replace nextlines with breaks (on input)
...
//    ['fontstyle'],
//    ['linebreak'],
    ['br','bold','italic','underline','separator'],
//  ['strikethrough','subscript','superscript','separator'],
    ['justifyleft','justifycenter','justifyright','justifyfull','separator'],
    ['OrderedList','UnOrderedList','Outdent','Indent','separator'],
    ['forecolor','backcolor','separator'],
    ['HorizontalRule','Createlink','InsertImage','InsertLocimg','InsertTable','htmlmode','separator'],
...
this.btnList = {
    ...
    "br":           ['BR',                 '<BR>',               'editor_action(this.id)',  'ed_html_br.gif'],
    ...
    "insertlocimg":   ['InsertLocimg',         'Insert Local Image', 'editor_action(this.id)',  'ed_image_loc.gif'],
...
function editor_generate(objname,userConfig) {
  ...
  if (config.plaintextInput) {     // replace nextlines with breaks
   ...
   var delim = "/gi";
		var regexp="";// = eval("/" + 'src="' + delim);
   var replaceStr="";// = 'src="';

   //var regexp = eval("/" + "src='" + delim);

		return contents.replace(regexp, replaceStr);
	}

  var areacontents = document.all[objname].value;
  areacontents = doReplace(areacontents);
  //areacontents = areacontents.replace('src', 'srcf');
  document.all[objname].value = areacontents;
...
function editor_action(button_id) {
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
    ...
    // insert local image
    else if (cmdID.toLowerCase() == 'insertlocimg'){
      showModalDialog(_editor_url + "popups/insert_localimg.html", editdoc, "resizable: no; help: no; status: no; scroll: no; ");
      //alert('ssdfefgdsfg');
    }
