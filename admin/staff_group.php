<?php
	...
	$PAGE_HEADER['ru'] = "Редактировать Группы Сотрудников";
	...
	$THIS_TABLE = $TABLE_STAFF_GROUP;
	$THIS_TABLE_LANG = $TABLE_STAFF_GROUP_LANGS;
	...
	switch( $action )
	{
    	case "add":
    		...
    		$query = "INSERT INTO $THIS_TABLE ( icon_filename, sort_num )
    			VALUES ('".addslashes($orglogo)."', '$orgsort')";
		...
		case "update":
			...
			if(!mysqli_query($upd_link_db,"UPDATE $THIS_TABLE SET icon_filename='".addslashes($orglogo)."', sort_num='$orgsort' WHERE id='".$item_id."'"))
			{
				echo "<b>".mysqli_error($upd_link_db)."</b>";
			}
...
?>
    ...
    <tr><th>&nbsp;</th><th>Иконка</th><th>Название типа</th><th>Функции</th></tr>
    <?
    	$found_items = 0;
		if( $res = mysqli_query($upd_link_db,"SELECT p1.*, p2.type_name, p2.descr
			FROM $THIS_TABLE p1, $THIS_TABLE_LANG p2
