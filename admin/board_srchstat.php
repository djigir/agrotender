...
function checkDtFormat($dt)
{
	...
	$PAGE_HEADER['ru'] = "Статистика поиска на доске объявлений";
	$PAGE_HEADER['en'] = "Board search stat";
	...
	switch( $action )
	{
		case "delete":
			...
			{
    			if(!mysqli_query($upd_link_db,"DELETE FROM $TABLE_ADV_SEARCH WHERE id=".$com_id[$i]." "))
				{
		...
		case "deleteitem":
            ...
            if(!mysqli_query($upd_link_db,"DELETE FROM $TABLE_ADV_SEARCH WHERE id=".$item_id." "))
			{
        		echo "<b>".mysqli_error($upd_link_db)."</b>";
    		}
    		break;
		...
		case "update":
			...
			if(!mysqli_query($upd_link_db,"UPDATE $TABLE_ADV_SEARCH SET keyword='".addslashes($newstitle)."', rating='".addslashes($newsrate)."' WHERE id='".$item_id."'"))
			{
				echo "<b>".mysqli_error($upd_link_db)."</b>";
			}
			break;
    ...
    if( $mode == "edit" )
    {
		...
		if($res = mysqli_query($upd_link_db,"SELECT * FROM $TABLE_ADV_SEARCH WHERE id='$item_id'"))
		{
...
<?php
    	$found_news = 0;
		if( $res = mysqli_query($upd_link_db,"SELECT s1.*, t1.title as topictit
			FROM $TABLE_ADV_SEARCH s1
			LEFT JOIN $TABLE_ADV_TOPIC t1 ON s1.topic_id=t1.id 
			ORDER BY s1.rating DESC, s1.add_date DESC") )
		{
			while($row=mysqli_fetch_object($res))
			{
				...
				echo "<tr>
					...
					<td style=\"padding: 2px 10px 2px 10px;\"><b>".stripslashes($row->keyword)."</b></td>
					<td style=\"padding: 2px 10px 2px 10px;\">".( $row->topictit != null ? stripslashes($row->topictit) : '' )."</td>
