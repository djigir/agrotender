<?php
	...
	*/
	...
	$orgcult = GetParameter("orgcult", 0);
	...
	switch( $action )
	{
		case "delete":	
			if( $item_id != 0 )
			{
				$query = "DELETE FROM $TABLE_SEO_TITLES WHERE id='$item_id'";
		...
		case "addnew":			
			...
			if( ($orgcult != 0) || ($orgobl != 0) )
			{
				// Check if it exists
				$found_id = 0;
				$query = "SELECT * FROM $TABLE_SEO_TITLES WHERE pagetype='$SEO_PAGETYPE_TRADER' AND cult_id='$orgcult' AND obl_id='$orgobl' AND type_id='$orgtype' AND sortmode_id='$sort_id' 
				...
				if( $found_id == 0 )
				{
					if( !mysqli_query($upd_link_db,"INSERT INTO $TABLE_SEO_TITLES (pagetype, page_h1, page_title, page_keywords, page_descr, content_text, content_words,
						modify_date, sect_id, cult_id, obl_id, type_id, add_date, lang_id,
						tpl_items_title, tpl_items_keywords, tpl_items_descr, tpl_items_text, tpl_items_words,
						sortmode_id, filter_id, filter_val)
						VALUES ('$SEO_PAGETYPE_TRADER', '".addslashes($orgh1)."','".addslashes($orgtitle)."','".addslashes($orgkeyw)."','".addslashes($orgdescr)."',
							'".addslashes($orgcont)."', '".addslashes($orgwords)."', 
							NOW(), '0', '".$orgcult."', '$orgobl', '$orgtype', NOW(), '$LangId',
	...
	*/
	...
	if( $mode == "edit" )
	{
		...
		*/
		...
		$query0 = "SELECT * FROM $TABLE_TRADER_PR_CULTS_LANGS c1 WHERE c1.item_id='$orgsectid' AND c1.lang_id='$LangId'";
		//echo $query0."<br>";
...
?>
	...
	<tr><td class="ff">Тип:</td><td class="fr"><b><?=($orgtype == 3 ? 'Форварды' : ($orgtype == 1 ? "Продажи" : "Закупки" ));?></b></td></tr>	
...
<?php
	...
	else
	{
		...
		$query = "SELECT t1.*, case when s1.id IS NOT NULL then s1.name else '' end as cultname 
			FROM $TABLE_SEO_TITLES t1
			LEFT JOIN $TABLE_TRADER_PR_CULTS_LANGS s1 ON t1.cult_id=s1.item_id AND s1.lang_id='$LangId' 
			WHERE t1.pagetype='$SEO_PAGETYPE_TRADER' 
			ORDER BY cultname, t1.cult_id, t1.obl_id, t1.type_id";
		if( $res = mysqli_query($upd_link_db,$query) )
		{
			while( $row = mysqli_fetch_object($res) )
			{
				...
				$it['type'] = ($row->type_id == 3 ? 'Форварды' : ( $row->type_id == 1 ? "Продажи" : "Закупки" ));
...
?>
	<h3>SEO Метаданные раздела трейдеров</h3>
...
?>
	...
	<h3>Добавить новые SEO данные в раздел Трейдеров</h3>
	<table align="center" cellspacing="0" cellpadding="1" border="0" class="tableborder">
    <tr><td>
    	<table width="100%" cellspacing="1" cellpadding="1" border="0">
	<form action="<?=$PHP_SELF;?>" name="catfrm" method="POST">
	...
	<tr>
    	...
    	<td class="fr">
    		<select name="orgcult">
    			<option value="0">--- Культура не выбрана ---</option>
<?php
			$cultgroups = Trader_GetCultGroups($LangId, "", $acttype);

			for( $i=0; $i<count($cultgroups); $i++ )
			{						
				echo '<option value="0">--- '.$cultgroups[$i]['name'].' ---</option>';
				//echo '<h3 data-cgrid="fgr'.$cultgroups[$i]['id'].'">'.$cultgroups[$i]['name'].'</h3>';		
				$cults = Trader_GetCults($LangId, 0, "", false, $cultgroups[$i]['id']);
				if( count($cults) > 0 )
				{
					for( $j=0; $j<count($cults); $j++ )
					{						
						echo '<option value="'.$cults[$j]['id'].'"'.( $orgcult == $cults[$j]['id'] ? ' selected' : '' ).'> &nbsp;&nbsp;&nbsp; '.$cults[$j]['name'].'</option>';
						//echo '<li class="chk-cults"><input id="culto'.$cults[$j]['id'].'" type="checkbox" name="cultlist[]" value="'.$cults[$j]['id'].'" '.($cult_checked ? ' checked' : '').'><label for="culto'.$cults[$j]['id'].'"><a href="'.$URL.'">'.$cults[$j]['name'].'</a> ('.$prnum.')</label></li>';						
					}
				}
			}			
?>
...
?>
	...
	<tr>
    	...
    	<td class="fr">
    		<select name="orgtype">
    			<option value="0" <?=($orgtype == 0 ? " selected" : "");?>>Закупки</option>
				<option value="1" <?=($orgtype == 1 ? " selected" : "");?>>Продажи</option>
				<option value="3" <?=($orgtype == 3 ? " selected" : "");?>>Форварды</option>
    		</select>
    	</td>
    </tr>
	<?php
