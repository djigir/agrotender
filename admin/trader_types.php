<?php
	...
	switch( $action )
	{
		case "edit":
			...
			if($res = mysqli_query($upd_link_db,"SELECT p1.* FROM $TABLE_TRADER_TYPES p1 WHERE p1.id='$item_id'"))
			{
    	...
    	case "add":
    		...
    		$query = "INSERT INTO $TABLE_TRADER_TYPES ( icon_filename, url, sort_num, name )
    			VALUES ('".addslashes($orglogo)."', '".addslashes($orgurl)."', '".$orgsort."', '".addslashes($orgname)."')";
		...
		case "delete":
			...
			{
    			if(!mysqli_query($upd_link_db,"DELETE FROM $TABLE_TRADER_TYPES WHERE id=".$items_id[$i]." "))
				{
        			echo "<b>".mysqli_error($upd_link_db)."</b>";
    			}
    			else
    			{
                    if( !mysqli_query($upd_link_db, "DELETE FROM $TABLE_TRADER_TYPES2ITEMS WHERE type_id='".$items_id[$i]."'" ) )
                    {
                       echo mysqli_error($upd_link_db);
                    }
                }
		...
		case "deleteitem":
			$item_id = GetParameter("item_id", "0");
            if(!mysqli_query($upd_link_db,"DELETE FROM $TABLE_TRADER_TYPES WHERE id=".$item_id." "))
			{
        		echo "<b>".mysqli_error($upd_link_db)."</b>";
    		}
    		else
    		{
                if( !mysqli_query($upd_link_db, "DELETE FROM $TABLE_TRADER_TYPES2ITEMS WHERE type_id='".$item_id."'" ) )
                {
                       echo mysqli_error($upd_link_db);
                }
            }
		...
		case "update":
			...
			if(!mysqli_query($upd_link_db,"UPDATE $TABLE_TRADER_TYPES SET sort_num='".$orgsort."', url='".addslashes($orgurl)."', 
				icon_filename='".addslashes($orglogo)."', name='".addslashes($orgname)."' WHERE id='".$item_id."'"))
			{
				echo "<b>".mysqli_error($upd_link_db)."</b>";
			}
...
?>
    <h3>Перечень типов трейдеров</h3>
    ...
    <?
		$found_items = 0;
		if( $res = mysqli_query($upd_link_db,"SELECT p1.* FROM $TABLE_TRADER_TYPES p1 ORDER BY p1.sort_num, p1.name") )
		{
			while($row=mysqli_fetch_object($res))
			{
				...
				echo "</td>
				*/
				echo "<td style=\"padding: 1px 10px 1px 10px\">
		...
		if( $found_items == 0 )
		{
			echo "<tr><td colspan=\"4\" align=\"center\"><br />В базе нет типов<br /><br /></td></tr>
			<tr><td colspan=\"4\" bgcolor=\"#EEEEEE\"><img src=\"spacer.gif\" width=\"1\" height=\"1\" alt=\"\" /></td></tr>";
        }
        else
        {
    ...
    <br /><br />
    <h3>Добавить новый тип трейдера</h3>
    <table align="center" cellspacing="0" cellpadding="1" border="0" class="tableborder">
    <tr><td>
    	<table width="100%" cellspacing="1" cellpadding="1" border="0">
    <form action="<?=$PHP_SELF;?>" name="catfrm" method="POST">
