...
<div class="new_container">
  <div class="new_feed">
    <button class="new_feed-button prev"></button>
    <div class="swiper-container">
        <div class="swiper-wrapper">
        {foreach $feed as $item}
        {assign var=rubrics_count value=$item['r']|@count}
            <div class="swiper-slide " style="margin-right:0px !important; ">
                <a href="/kompanii/comp-{$item['company_id']}" class="new_feed-item">
                  <span class="new_feed-item-title">{$item['company']|unescape|truncate:25:"..":true}</span>
                  <div class="new_feed-item-state confirmed">{$item['onchange']}</div> 
                  

              		<ul>
                   
                                   {foreach from=$item['r'] item=rubrics key=rubric} <li>
              {if $rubrics@iteration eq 3}
              {break}
              {/if}
                      <span>{$rubric|unescape|truncate:18:"..":true}</span>
                  ...
                  <div class="new_feed-item-bottom">
                    <span class="more">{if ($rubrics_count - 2) > 0}+ ещё {$rubrics_count - 2}{/if}</span>
    ...
    <button class="new_feed-button next"></button>
  </div>
</div>
...
{foreach name=group from=$group item=trader}
  <div class="new_traders {if $trader['top'] eq '2'} vip{/if}">
{foreach name=group from=$group item=trader}
    <div class="traders__item-wrap">
      <a href="/kompanii/comp-{$trader['id']}{if $section neq 'buy'}-prices?type=1{else}{/if}" class="traders__item">
        <div class="traders__item__header">
        	{if $trader['top'] eq 2} <span class="vip">VIP</span> {/if}
...
{/if}
    {foreach name=group from=$group item=trader}
      ...
      <div class="row newTraderItem mx-0{if $trader['top'] eq '2'} top{/if}">
        
        {if $trader['top'] eq 2} <span class="vipblock">VIP</span> {/if}
...
<!--   <div class="new_container">
    <div class="new_traders">
      <div class="traders__item-wrap">
        <a href="#" class="traders__item yellow">
          <div class="traders__item__header">
