{$submenu=[
  'Закупки'=>'/traders/region_ukraine',
  'Продажи'=>'/traders_sell',
  'Аналитика закупок'=>'/traders_analitic/region_ukraine',
  'Аналитика продаж'=>'/traders_analitic_sell'
]}
{* <!-- START_BLOCK: ТОЛЬКО ДЛЯ МОБИЛЬНОЙ ВЕРСИИ --> *}
    ...
    <div class="filters arrow-t">
      <div class="step-1 stp">
        <div class="position-relative scroll-wrap">
          ...
          <a class="mt-3 p-4 content-block filter filter-type d-flex justify-content-between" href="#" type="_forwards">
            <span>Форварды</span>
      ...
      <div class="step-2 stp h-100">
        ...
        <div class="scroll">
        {foreach $submenu as $name => $path}
          <a class="rubric px-4 py-3 my-3 content-block d-flex justify-content-between" href="{$path}">
            <span>{$name}</span>
            <span><i class="far fa-chevron-right"></i></span>
          </a>
        {/foreach}
...
{* <!-- END_BLOCK: ТОЛЬКО ДЛЯ МОБИЛЬНОЙ ВЕРСИИ --> *}

<div class="d-none d-sm-block container mt-3">
  <ol class="breadcrumbs small p-0">
    <li><a href="/"><span>Главная</span></a></li>
    {if $rubric eq null && $region eq null && $port eq null && $onlyPorts eq null}
    <i class="fas fa-chevron-right extra-small"></i>
    <li><h1>{if $h1 neq ''}{$h1}{else}Форварды трейдеров в {if $region['id'] eq null}Украине{else}{$region['parental']} области{/if}{/if}</h1></li>
    {else}
    <i class="fas fa-chevron-right extra-small"></i>
    <li><a href="/traders_forwards"><span>Форварды</span></a></li>
      {if $rubric neq null && $region neq null}
    <i class="fas fa-chevron-right extra-small"></i>
    <li><a href="/traders_forwards/{if $rubric neq null}{$rubric['translit']}{/if}"><span>Форварды {if $rubric neq null}{$rubric['name']}{/if}</span></a></li>
      {/if}
      {if !$h1 }
    <i class="fas fa-chevron-right extra-small"></i>
    <li><a href="/traders_forwards/{if $rubric neq null}{$rubric['translit']}{elseif $port neq null}tport_{$port['translit']}{/if}"><span>Цена {if $rubric neq null}{$rubric['name']}{elseif $port neq null}{$port['name']}{/if}</span></a></li>
      {/if}
    <i class="fas fa-chevron-right extra-small"></i>
    <li><h1>{$h1}</h1></li>
    {/if}
        ...
        <div class="swiper-slide">
          <div class="feed-item row">
            <div class="col-12 d-flex align-items-center">
              <div class="circle{if $item@iteration is div by 2} two{/if} d-inline-block mr-1"></div> <a href="/kompanii/comp-{$item['company_id']}-forwards" class="title lh-1">{$item['company']|unescape|truncate:25:"..":true}</a>
            </div>
            ...
            <div class="col-12 d-flex align-items-center justify-content-between mt-1">
              <a href="/kompanii/comp-{$item['company_id']}-forwards" class="more">{if ($rubrics_count - 2) > 0}+ ещё {$rubrics_count - 2}{/if}</a>
  ...
  <div class="content-block mt-3 py-3 px-3">
    <div class="btn-group position-relative w-100 ">
              ...
              <div class="row">
                ...
                {foreach from=$rubrics item=groups key=group_id}
                  ...
                  <div class="col-auto rubricGroup pr-0 mr-3 group-{$group_id}">
                    {foreach from=$group item=r}
                    <a rid="{$r['id']}" class="rubricLink{if $r['name'] eq $rubric['name']} active{/if}" href="/traders_forwards{if $region neq null}/region_{$region['translit']}{else if $port neq null or $onlyPorts eq 'yes'}/tport_{if $port neq null}{$port['translit']}{else}all{/if}{else}/region_ukraine{/if}/{$r['translit']}{if $currency !== null}?currency={$currency['code']}{/if}{if $viewmod eq 'nontbl'}{if $currency !== null}&{else}?{/if}viewmod=nontbl{/if}">
      ...
      <div class="dropdown-wrapper position-absolute regionDrop">
        <div class="dropdown">
          <span class="d-block">
            <a class="regionLink d-inline-block{if $port eq null && $onlyPorts eq null && $region eq null} text-muted disabled{/if}" href="/traders_forwards/region_ukraine{if $rubric neq null}/{$rubric['translit']}{/if}{if $currency !== null}?currency={$currency['code']}{/if}{if $viewmod eq 'nontbl'}{if $currency !== null}&{else}?{/if}viewmod=nontbl{/if}">
            <span>Вся Украина</span>
          </a>
          <a class="regionLink d-inline-block{if $region['id'] eq 1} text-muted disabled"{else}"{/if} href="/traders_forwards/region_crimea{if $rubric neq null}/{$rubric['translit']}{/if}{if $currency !== null}?currency={$currency['code']}{/if}{if $viewmod eq 'nontbl'}{if $currency !== null}&{else}?{/if}viewmod=nontbl{/if}">
            <span>АР Крым</span>
          </a>
          </span>
          <hr class="mt-1 mb-2">
          <div class="section text-left">
            ...
            <div class="row">
              ...
              <div class="col">
                ...
                <a class="regionLink{if $c['name'] eq $region['name']} active{/if}" href="/traders_forwards/region_{$c['translit']}{if $rubric neq null}/{$rubric['translit']}{/if}{if $currency !== null}?currency={$currency['code']}{/if}{if $viewmod eq 'nontbl'}{if $currency !== null}&{else}?{/if}viewmod=nontbl{/if}">
          ...
          <br>
          <span class="d-block">
            <a class="regionLink d-inline-block{if $onlyPorts eq 'yes'} text-muted disabled"{else}"{/if} href="/traders_forwards/tport_all{if $rubric neq null}/{$rubric['translit']}{/if}{if $viewmod eq 'nontbl'}?viewmod=nontbl{/if}">
            <span>Все порты</span>
          </a>
          </span>
          <hr class="mt-1 mb-2">
          <div class="section text-left">
            ...
            <div class="row">
              ...
              <div class="col">
                {foreach from=$col item=c}
                <a class="regionLink{if $c['name'] eq $port['name']} active{/if}" href="/traders_forwards/tport_{$c['translit']}{if $rubric neq null}/{$rubric['translit']}{/if}{if $viewmod eq 'nontbl'}?viewmod=nontbl{/if}">
                  <span>{$c['name']}</span>
                  <!--<span class="float-right right"><i class="far fa-chevron-right"></i></span>-->
                </a>
                {/foreach}
      ...
      <div class="d-flex align-items-center">
        <a class="text-center filter-icon mr-3{if $viewmod eq 'nontbl'} active{/if}" rel="nofollow" href="{$smarty.server.SCRIPT_URL}?{if $currency !== null}?currency={$currency['code']}&{/if}viewmod=nontbl"><i class="fas fa-th-large"></i></a>
...
<div class="d-sm-none container pt-2 pt-sm-4">
  {if $rubric neq null}
  <span class="searchTag d-inline-block">{$rubric['name']} <a href="/traders_forwards{if $region neq null}/region_{$region['translit']}{else}/region_ukraine{/if}{if $currency neq null}?currency={$currency['code']}{/if}"><i class="far fa-times close ml-2"></i></a></span>
  {/if}
  {if $region neq null}
  <span class="searchTag d-inline-block">{if $region['id'] eq 1}АР Крым{else}{$region['name']} область{/if} <a href="/traders_forwards{if $rubric neq null}/{$rubric['translit']}{/if}{if $currency neq null}?currency={$currency['code']}{/if}"><i class="far fa-times close ml-2"></i></a></span>
  {/if}
  {if $currency neq null}
  <span class="searchTag d-inline-block">{$currency['name']} <a href="/traders_forwards{if $region neq null}/region_{$region['translit']}{else}/region_ukraine{/if}{if $rubric neq null}/{$rubric['translit']}{/if}"><i class="far fa-times close ml-2"></i></a></span>
  {/if}
</div>
{/if}
{* <!-- END_BLOCK: ТОЛЬКО ДЛЯ МОБИЛЬНОЙ ВЕРСИИ --> *}
  ...
  <div class="row mt-sm-0 pt-sm-0 mb-sm-4">
    <div class="position-relative w-100">
      <div class="col-12 col-md-9 float-md-right text-center text-md-right">
        ...
        <a id="addCompanny" href="/add_buy_trader" class="top-btn btn btn-warning align-items-end">
      ...
      <div class="row newTraderItem mx-0{if $trader['top'] eq '1'} top{/if}">
        ...
        <div class="col-12 px-0 d-flex justify-content-between align-items-center px-2 pb-2 rating text-center">
          <a class="stars" href="/kompanii/comp-{$trader['id']}-reviews">
            {for $i=1 to 5}<i class="{if $i <= $trader['review']['rating']}fas{else}far{/if} fa-star"></i>{/for}
          </a>
...
<div class="container mt-0 mt-sm-3">
  ...
  {if $rubric eq null and $tableList eq null}
...
<div class="container pb-5 pb-sm-4 pt-4 mb-4 scroll-x">
  <table class="sortTable sortable{if !$detect->isMobile()} dTable{/if}" cellspacing="0">
    ...
    <thead>
      <tr>
        <th>Компании</th>
      {if $currency eq null or $currency['code'] eq 'uah'}
        <th class="sth">UAH <i class="fas fa-sort" style="font-size: 12px;"></i></th>
      {/if}
      {if $currency eq null or $currency['code'] eq 'usd'}
    ...
    <tbody>
      ...
      <tr class="t-sub" cspan="1">
        ...
        <td><div class="d-flex align-items-center justify-content-center"> <span class="price">0</span> </div></td>
        <td><div class="d-flex align-items-center justify-content-center"> <span class="price">0</span> </div></td>
        <td><span data-date="20170802"></span></td>
        <td></td>
      </tr>
      {/if}
      <tr{if $row['top'] eq 1} class="vip"{/if}>
        <td>
          <a class="d-flex align-items-center" href="/kompanii/comp-{$row['id']}-forwards">
        ...
        <td class="usd">
          ...
          <div class="d-flex align-items-center justify-content-center">
            ...
            {if $row['prices']['usd']['comment'] }
          <span class="d-block text-muted extra-small">{$row['prices']['usd']['comment']}</span>
            {/if}
          {/if}
        </td>
        {/if}
        <td data-sorttable-customkey="{$row['date']|date_format:"%Y%m%d"}"><span data-date="{$row['date']|date_format:"%Y%m%d"}">{$row['date_show']}</span></td>
        <td>
      ...
      <tr{if $row['top'] eq 1} class="vip"{/if}>
        <td>
          <div class="d-flex align-items-center price-div">
            ...
            <a class="flex-1" href="/kompanii/comp-{$row['id']}-forwards">
      ...
      <tr class="t2 {if $row['top'] eq 1} vip{/if}">
        <td style="border-bottom: 1px solid #295ca1;">
          <div class="d-flex align-items-center justify-content-center">
            <span data-toggle="tooltip" data-placement="top" class="d-block">{$row['date_show']}</span>
            <a href="/kompanii/comp-{$row['id']}-forwards" class="d-block flex-1">
...
{if ($traders eq null && $viewmod eq 'nontbl') or ($viewmod eq null && $tableList eq null && $rubric neq null)}
  ...
  <div class="content-block p-5">
    ...
    <span class="all">Предлагаем Вам ознакомиться с общим <a href="/traders_forwards">списком трейдеров</a></span>
  </div>
</div>
{/if}
