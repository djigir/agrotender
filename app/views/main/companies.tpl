...
<div class="d-none d-sm-block container mt-3">
  ...
  <div class="row mt-4 pt-3">
      <div class="col-12 col-sm-4 float-left mt-4 mt-md-0 d-flex d-sm-block">
        <h2 class="d-inline-block text-uppercase">{if $query neq null}Поиск: {$query}{else}Список компаний{/if}</h2>
...
<div class="container pb-4 companies">
  ...
  <div class="row content-block companyItem mx-0 mt-4 pt-3 pb-1 py-sm-3 px-1 {if $company['top']==1}companyTop{/if} {if $company['top']==2}companyTop{/if}"
  {if $company['top'] == 2} style ="overflow:hidden;"{/if}}>
            {if $company['top']== 2} <div class="ribbonComp">VIP</div> {/if}
    <div class="row mx-0 w-100">
      <div class="col-auto pr-0 pl-2 pl-sm-3">
        <div class="row m-0">
          <div class="col-12 pl-0 pr-0 pr-sm-2">
            <a href="/kompanii/comp-{$company['id']}"><img class="companyImg" src="{if $company['logo_file'] neq null}/{$company['logo_file']}{else}/app/assets/img/no-image.png{/if}"></a>
          </div>
        </div>
      ...
      <div class="col">
        <div class="row lh-1">
          <div class="col">
            <span class="title"><a href="/kompanii/comp-{$company['id']}">{$company['title']|trim}</a></span>
          </div>
        </div>
        ...
        <div class="row d-none d-sm-flex">
          <div class="col mt-1">
            <p class="desc">{$company['short']|trim}</p>
          </div>
        </div>
      ...
      <div class="d-none d-md-block col-md-3">
        <div class="companySticker">
          {if $company['phone'] neq null}
          <span>{$company['phone']}</span>
          {/if}
          {if $company['phone2'] neq null}
          <span>{$company['phone2']}</span>
          {/if}
          {if $company['phone3'] neq null}
