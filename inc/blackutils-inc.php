...
////////////////////////////////////////////////////////////////////////////////
	...
	////////////////////////////////////////////////////////////////////////////////
	// Functions
function Black_BuildUrl_PHP($LangId, $pi=0, $pn=20, $nid=0, $mod="", $y=0, $m=0, $groupurl="")
{
	global $PHP_SELF;
	// sample link styles
	//   news.php
...
function Black_BuildUrl_HTML($LangId, $pi=0, $pn=20, $nid=0, $mod="", $y=0, $m=0, $groupurl="")
{
	// sample link styles
	//   news/index.html
	//   news/page_2_20.html
	...
	*/
	...
	if( ($nid == 0) || ($nid == "") )
	{
		...
		{
			$url .= ( $pi > 0 ? "page_".$pi.".html" : "index.html" );

			if( $url == "black/index.html" )
				$url = "black.html";
		}
	}
	else
...
function Black_BuildUrl($LangId, $pi=0, $pn=20, $nid=0, $mod="", $y=0, $m=0, $groupurl="")
{
	...
	return (
		$WWW_LINK_MODE == "php" ?
			$wwwlink.Black_BuildUrl_PHP($LangId, $pi, $pn, $nid, $mod, $y, $m, $groupurl) :
			$WWWHOST.Black_BuildUrl_HTML($LangId, $pi, $pn, $nid, $mod, $y, $m, $groupurl)
		);
}

function Black_ItemsNum($ngroup = -1)
{
	global $TABLE_BLNEWS;
	...
	if( $ngroup != -1 )
	{
   	...
   	$query = "SELECT count(*) as totnews FROM $TABLE_BLNEWS $cond";
...
function Black_Items($LangId, $ngroup=-1, $sortby="add", $anons_len=300, $pi=-1, $pn=20, $year=0, $month=0)
{
	global $TABLE_BLNEWS, $TABLE_BLNEWS_LANGS, $WWWHOST, $FILE_DIR, $DEF_ENC;
	...
	$query = "SELECT m1.*, m2.title, m2.content, DAYOFMONTH(dtime) as day, MONTH(dtime) as month, YEAR(dtime) as year
   			FROM $TABLE_BLNEWS m1, $TABLE_BLNEWS_LANGS m2
...
function Black_ItemInfo($LangId, $id)
{
	global $TABLE_BLNEWS, $TABLE_BLNEWS_LANGS, $TABLE_TORG_BUYERS, $WWWHOST, $FILE_DIR;
	...
	if( $res = mysqli_query($upd_link_db, $query ) )
	{
	    while( $row = mysqli_fetch_object($res) )
	    {
			...
			$ninfo['author_id'] = $row->author_id;
			$ninfo['author'] = "";

			$query1 = "SELECT * FROM $TABLE_TORG_BUYERS WHERE id=".$row->author_id;
			if( $res1 = mysqli_query($upd_link_db, $query1 ) )
			{
				while( $row1 = mysqli_fetch_object( $res1 ) )
				{
					$ninfo['author'] = stripslashes($row1->name);
				}
...
function Black_GetFirstItemDate($ngroup=-1)
{
	global $TABLE_BLNEWS;
...
function Black_CommentNum($comp_id)
{
	global $TABLE_BLNEWS_COMMENT;

	$comments_num = 0;

	$query = "SELECT count(*) as totcomments FROM $TABLE_BLNEWS_COMMENT WHERE item_id='$comp_id' AND visible=1 ";
	if( $res = mysqli_query($upd_link_db, $query ) )
	{
		while( $row = mysqli_fetch_object( $res ) )
		{
...
function Black_CommentAvgRate($comp_id)
{
	global $TABLE_BLNEWS_COMMENT;

	$resp_avg = 0;

	$query = "SELECT avg(rate) as avgrate FROM $TABLE_BLNEWS_COMMENT WHERE item_id='$comp_id' AND visible=1 ";
	if( $res = mysqli_query($upd_link_db, $query ) )
	{
		while( $row = mysqli_fetch_object( $res ) )
		{
...
function Black_Comments($LangId, $comp_id, $pi=-1, $pn=20, $dtsqltpl="")
{
	global $TABLE_BLNEWS_COMMENT, $TABLE_BLNEWS_COMMENT_LANGS;
	...
	$query = "SELECT c1.*, DATE_FORMAT(c1.add_date, '$dtsqlfmt') as dtstr, c2.content FROM $TABLE_BLNEWS_COMMENT c1
		INNER JOIN $TABLE_BLNEWS_COMMENT_LANGS c2 ON c1.id=c2.item_id AND c2.lang_id='$LangId'
