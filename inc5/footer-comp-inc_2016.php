...
?>
		...
		<div id="right">
			<div class="lbl">
				...
				<div class="lbl-mid">
		<?php
		$yand="yaCounter117048.reachGoal('LentaRightKomp');";
			$its = Array();
			$any_type_id = -1;
			if( $COMP_ID != 0 )
			{
                ...
                for( $i=0; $i<count($its); $i++ )
                {
                	...
                	echo '<div class="rit'.($i % 2 == 0 ? ' rit-even' : '').($css_type_class != "" ? ' '.$css_type_class : '').'"><a href="'.$GOURL.'" onclick="'.$yand.'">'.$its[$i]['title'].'</a><p>'.$its[$i]['difstr'].'</p></div>';
                }
			}
		...
		?>
			...
			*/
			...
			if( $any_type_id > 0 )
			{
				...
				echo '<div class="more"><a href="'.$CLNK.'" onclick="'.$yand.'">посмотреть все</a></div>';
			}
		?>
				</div>
			</div>
