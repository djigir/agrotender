...
?>
	<link rel="stylesheet" type="text/css" href="<?=$WWWHOST;?>css5/all5.css" />
...
<?php
	...
	*/
?>
	<script type="text/javascript" src="<?=$WWWHOST;?>js5/main17.js"></script>
<?php	
	if( $pname == "addtorg" )
	{
?>
...
<?php
	...
	{
?>
	var userchecked = <?=( $UserSmsChecked ? '1' : '0' );?>;
<?php
	}
?>
			...
			for($i=0; $i<(count($menus)-1); $i++)
			{
				...
				{					
					echo '<li class="hmm_li  active" id="m'.($i+1).'">
							<a '.($menus[$i]['name'] != 'Элеваторы' ? '' : ' class="tgm-btn-18"').' href="'.$menus[$i]['link'].'">'.$menus[$i]['name'].'</a>										
					</li>';
				}
				else
				{
		...
		?>
			...
			<div class="header-right">
				...
				<?php
					...
					echo '<a '.( $UserId != 0 ? ' href="#" id="msngrreq"' : ' id="logotherlnk1" href="'.Page_BuildUrl($LangId, "buyerlog", "").'" ' ).' class="btn btn-light btn-mes tgm-btn-11">Продать трейдеру</a>';
					...
					echo '<a href="'.Page_BuildUrl($LangId, "board", "addpost").'" class="btn btn-light btn-iadv tgm-btn-2"><span>Объявление</span></a>';
				
					if( $UserId == 0 )
					{
						echo '<a href="'.Page_BuildUrl($LangId,"buyerlog","").'" class="btn btn-dark btn-ilog tgm-btn-5"><span>Вход</span></a>';
...
сырья в разделе Мессенджер">'.$msngr_new_num.'</b></a>' : '').'
						...
						*/
						...
						<div class="drop_menu">
						<ul>
							<li><a class="tgm-btn-7" href="'.$WWWHOST.'bcab_posts.php?viewarc=0&viewtype=0&sortby=postid_down'.(( ($UserId>0) && (count($complist)>0) ) ? "?viewcomp=1" : "").'">Объявления</a></li>'. ( ($Buyer) ? ' 
							<li><a href="'.$WWWHOST.'bcab_prices.php">Таблица закупок</a></li>' : "" ).
							( ($Seller) ? ' 
							<li><a href="'.$WWWHOST.'bcab_prices.php?acttype=1">Таблица продаж</a></li>' : "" ).' 
							<li><a class="tgm-btn-8-4" href="'.$WWWHOST.'bcab_msngr.php'.( ($Buyer) ? '?viewdir=1"' : '"').'>Мессенджер </a>'.( $msngr_new_num > 0 ? '<span class="drop_menu-bubble">'.$msngr_new_num.'</span>' : '' ).'</li> 
							<li><a href="'.$WWWHOST.'bcab_personal_cont.php">Контакты</a></li>
							<li><a href="'.$WWWHOST.'bcab_addfund.php">Пополнить баланс</a></li>
							<li><a href="'.$WWWHOST.'bcab_tarif_plimit.php">Увеличить лимит объявлений</a></li>
...
<?php
	...
	if( $pname != "index" )
	{
		...
		//echo $PHP_SELF."<br>";
		if( ($PHP_SELF == "/traders.php") || ($PHP_SELF == "/traders_analitic.php") )
		{		
			//echo "Lenta";
