...
<?php
	...
	echo '<div class="srch-form srch-form-a" onclick="'.$yand.'">
		...
		<div><input class="tgm-btn-12" type="text" name="srchword" id="srchword" /><a id="srchbtn1" href="#" class="srch-bckgrnd" title="Искать"></a>		
		</div>
	...
	if( $TOTAL_POSTS > 0 )
	{
		...
		if( $TOTAL_PAGES > 1 )
		{
			...
			// First
			$P_F_LINK = Board_BuildUrl( $LangId, "list", $REGIONS_URL[$adobl], $adtype, $adtopic, 1, $pn );
			// Last
			$P_L_LINK = Board_BuildUrl( $LangId, "list", $REGIONS_URL[$adobl], $adtype, $adtopic, $TOTAL_PAGES, $pn );
			// Prev
			$P_P_LINK = Board_BuildUrl( $LangId, "list", $REGIONS_URL[$adobl], $adtype, $adtopic, ($pi > 1 ? $pi-1 : 1), $pn );
			// Next
			$P_N_LINK = Board_BuildUrl( $LangId, "list", $REGIONS_URL[$adobl], $adtype, $adtopic, ($pi < $TOTAL_PAGES ? $pi+1 : $TOTAL_PAGES), $pn );
			...
			//{
				for( $i=$vis1_num; $i<=$vis2_num; $i++ )
				{
					if( ($i == $vis1_num) && ($i > 2) )
					{
						$P_LINK = Board_BuildUrl( $LangId, "list", $REGIONS_URL[$adobl], $adtype, $adtopic, $i-1, $pn );
						$HTML_PAGES_LINKS .= '<a class="a-page" href="'.$P_LINK.'">...</a>';
					}

					$P_LINK = Board_BuildUrl( $LangId, "list", $REGIONS_URL[$adobl], $adtype, $adtopic, $i, $pn );
					if( $pi == $i )
	...
	*/			
	...
	$itst = Board_Posts( $LangId, $adtype, $adtopic, ($toptopic == $adtopic), $adobl, 0, 5, 0, 0, 0, 0, "", "", "", "", "", 0, "rand", "up", false, 0, false, "", false, true);
	...
	*/
	...
	{
		echo '<div class="target-advs">
		<h2 class="hdr hdr-sm">Топ объявления</h2>
		<a  href="'.$WWWHOST.'/info/limit_adv.html#p3" target="_blank">Как сюда попасть?</a>
		<div class="adv-lt">';
	
		$itst_byid = Array();
		for($i=0; $i<count($itst) && $i<5 ; $i++)
		{
	...
	echo '<h2 class="hdr hdr-sm">Обычные объявления</h2>
	<div class="adv-lt">';
	...
	for( $i=0; $i<count($its); $i++ )
	{
		...
		echo '<div class="adv-it'.($its[$i]['color'] > 0 ? ' adv-it-color' : '').'">
			<div class="adv-pic"><table><tr><td><a href="'.$POSTURL.'">'.$pic_html.'</a></td></tr></table></div>
			<div class="adv-i">								
	...
	if( $PAGE_CONT != "" )
	{
		echo '<div class="txt-blk txt-seo">'.$PAGE_CONT.'</div>';
	}
?>
