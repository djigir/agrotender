...
<div class="d-none d-sm-block container mt-5">
        ...
        <div class="dropdown-wrapper position-absolute typeDrop">
          <div class="dropdown">
            <div class="section text-left">
              ...
              <div class="row">
                <div class="col">
                  <a class="inline-link" href="/traders_forwards">
                    <span>Форварды</span>
  ...
  <a href="/traders/region_ukraine/kukuruza" class="popular__block">Кукуруза</a>
  <a href="/traders/region_ukraine/raps" class="popular__block">Рапс</a>
  ...
  <div class="row mt-sm-0 pt-sm-0 mb-sm-4">
    <div class="position-relative w-100">
      <div class="col-12 col-md-9 float-md-right text-center text-md-right">
        <a id="viberLink" style="background: linear-gradient(180deg, #8A78E7 0%, #7E65FF 100%); border: none; height: 35px" href="viber://pa?chatURI=agrotender_bot&text=Начать" class="top-btn btn btn-primary align-bottom mr-0 mr-sm-3 mb-3 mb-sm-0">
          <img src="/app/assets/img/company/viber4.svg" style="width: 18px"/>
          <span class="pl-1 pr-1">Продать в Viber</span>
        </a>
        <a id="telegramLnk" style="background: linear-gradient(180deg, #5CA9F1 0%, #44A4FF 100%); border: none; height: 35px" href="https://t.me/AGROTENDER_bot" class="top-btn btn btn-primary align-bottom mr-0 mr-sm-3 mb-3 mb-sm-0">
    ...
    <tbody>
      ...
      <tr{if $row['top'] eq 1} class="vip"{/if}>
        ...
        <td class="usd">
          ...
          {/if}
        {/if}
        <td data-sorttable-customkey="{$row['date']|date_format:"%Y%m%d"}"><span data-date="{$row['date']|date_format:"%Y%m%d"}">{if $smarty.now|date_format:"%Y-%m-%d" eq $row['date']}<span class="today">{$row['date2']}</span>{else}{$row['date2']}{/if}</span></td>
        <td>
